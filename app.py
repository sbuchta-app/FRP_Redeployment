...
# (Removed) Former DELTA_RWA_PP_B matrix. Redeployment intensity is now derived from donor/receiver risk weights.
...
DELTA_SPREAD_BPS_B: Dict[Tuple[str, str], float] = {
    ...
    ('B1_CRE_CORE', 'B2_STRUCTURED_WC'): +110.000,
...
class Cell:
    donor: str
    receiver: str
    rw_quotient: float  # donor RW / receiver RW (used as redeployment intensity factor)
...
def transition_product(donor: str, receiver: str) -> float:
    """Eligibility score per user rule-set.

    Step 1: q1 = receiver_net_margin / donor_net_margin
    Step 2: q2 = donor_risk_weight / receiver_net_margin
    Step 3: product = q1 * q2
    Step 4: eligible if product > 0 (finite)

    Note: receiver_net_margin is taken from ABS_NET_SPREAD_BPS_BY_B2 (bps),
    donor_net_margin from DONOR_NET_SPREAD_BPS (bps). Units cancel.
    """
    try:
        donor_nm = float(DONOR_NET_SPREAD_BPS.get(donor, 0.0))
        recv_nm = float(ABS_NET_SPREAD_BPS_BY_B2.get(receiver, 0.0))
        donor_rw = float(DONOR_RISK_WEIGHT.get(donor, 0.0))
    except Exception:
        return float("nan")
...
def is_transition_eligible(donor: str, receiver: str) -> bool:
    prod = transition_product(donor, receiver)
    return bool(prod > 0.0 and np.isfinite(prod))
...
def _eligible_cells_by_donor(
    ...
    eff = float(partner_share)

    for (d, r), dspr in delta_spread_bps.items():
        if d not in donors:
            continue

        # dspr is the Δnet spread (bps) for this donor→receiver transition
        ...
        # Eligibility per rule-set (positive product of quotients)
        if not is_transition_eligible(d, r):
            continue
        ...
        if receiver_split_by_donor is not None:
            if float(receiver_split_by_donor.get(d, {}).get(r, 0.0) or 0.0) <= 0.0:
                continue
        # Use RW quotient (donor RW / receiver RW) as the redeployment intensity factor
        rw_quotient = donor_rw / receiver_rw

        # economics term: redeployment value scaled by RW quotient and receiver spread
        delta_s_eff_bps = rw_quotient * eff * abs_spread_bps
...
def compute_max_roe_uplift_map(
    ...
    try:
        ...
        roe_df_maxcap = compute_roe_delta_transitions_greedy(
            ...
            upfront_fee_bps=upfront_fee_bps,  # very high target -> exhaust redeployment capacity
...
def simulate_offload(banks_df: pd.DataFrame, scenarios_bps: dict, srt_efficiencies: List[float]) -> pd.DataFrame:
    """Per-bank version of the 'simple offload' module in **steady-state** terms.

    The model is formulated as an end-state / steady-state level change (no explicit time horizon).
    As a result:
      - There is no 'Years' column.
      - 'Total' and 'Annual' outputs are identical (annual == steady-state level).
    Assumes CET1 capital is fixed: C = CET1_ratio * RWA.
    """
    rows = []
    for _, b in banks_df.iterrows():
        bank = b.get("Bank Name", b.get("Bank", ""))
        ...
        for sc_name, adv_bps in scenarios_bps.items():
            ...
            for eff in srt_efficiencies:
                ...
                rows.append({
                    ...
                    # Annual view (same in steady-state)
                    "Gross_RWA_Offload_EUR_bn_Yr": round(gross_rwa, 3) if np.isfinite(gross_rwa) else np.nan,
                    "Gross_RWA_Offload_pct_of_RWA_Yr": round((gross_rwa / R) * 100, 3) if (R > 0 and np.isfinite(gross_rwa)) else np.nan,
...
def compute_roe_delta_transitions_greedy(
    ...
    for i, row in df.iterrows():
        ...
        partner_share_dec = max(min(float(partner_share_pct) / 100.0, 0.999999), 0.0)
        ...
        if roe_target_bp_i <= 0 or target_profit_total <= 0:
            _solve = _run_once(0.0)
            profit_target_redeploy_yr = 0.0
        else:
            ...
            if _gap_lo >= 0.0:
                ...
                _hi = max(_pt_guess, target_profit_total, 1e-12)
...
with _tc1:
    ...
    scenario_bps = st.slider(
        "CET1-uplift target (bp, end-state)",
        min_value=0,
        max_value=400,
        value=168,
        step=1,
        help="Target CET1 uplift in basis points (bp), interpreted as an end-state / steady-state level.",
    )
...
with _tc2:
    st.subheader("Funded risk participation")

    partner_share = st.slider(
        "Strategic partner share (investor participation, %)" ,
        min_value=10,
        max_value=95,
        value=70,
        step=1,
        help="Investor share p in the funded risk participation (e.g., 70% means investor participates in 70% of the sub-portfolio economics / risk).",
        key="partner_share_slider",
    )

    skim_bps = st.slider(
        "Skim (bps, investor-share margin retained by bank)",
        min_value=0,
        max_value=200,
        value=50,
        step=1,
        help="Ongoing spread skim retained by the bank on the investor share (p).",
        key="skim_bps_slider",
    )

    servicing_bps = st.slider(
        "Servicing fee (bps, on total portfolio)",
        min_value=0,
        max_value=100,
        value=20,
        step=1,
        help="Ongoing servicing fee earned by the bank on the full portfolio notional (bank also services the investor share).",
        key="servicing_bps_slider",
    )

    upfront_fee_bps = st.slider(
        "Upfront/origination fee (bps, one-off; annualised over horizon)",
        min_value=0,
        max_value= 300,
        value= 100,
        step=1,
        help="One-off origination fee on the portfolio notional; the model annualises it over the chosen horizon.",
...
with _tc3:
    ...
    st.caption("Steady-state flow assumption: each year, this % of each donor sub-portfolio is renewed by new business. The transition engine allocates *annual* flow; partner share (p) applies to that flow.")

    roll_sme = st.slider("Rolling fraction — SME core (% p.a.)", 0, 25, 15, 1, key="roll_sme")
    roll_mid = st.slider("Rolling fraction — Mid-corp core (% p.a.)", 0, 25, 15, 1, key="roll_mid")
    roll_tf  = st.slider("Rolling fraction — Trade finance core (% p.a.)", 0, 25, 15, 1, key="roll_tf")
    roll_cre = st.slider("Rolling fraction — CRE core (% p.a.)", 0, 25, 15, 1, key="roll_cre")
    roll_cons = st.slider("Rolling fraction — Consumer finance core (% p.a.)", 0, 25, 15, 1, key="roll_cons")
    ...
    st.caption(
        ...
        "Transitions are eligible only if the model-implied eligibility score is positive (greyed-out cells are not selectable). "
        "For each donor (row), values are enforced to sum to 100% across eligible receivers (auto-normalised)."
    )
    ...
    # ---------------------------------------------------------------------
    # Eligibility is computed from the user rule-set (no hard-coded matrix):
    #   Step 1: q1 = receiver_net_margin / donor_net_margin
    #   Step 2: q2 = donor_risk_weight / receiver_net_margin
    #   Step 3: product = q1 * q2
    #   Step 4: eligible if product > 0 (finite)
    #
    # Non-eligible transitions are fixed at 0% and non-editable (greyed out).
    #
    # Row-sum constraint:
    #   For each donor (row), values across *eligible* receivers are forced to sum to 100%.
    #   We implement this by making the *last eligible receiver* a computed "remainder to 100%"
    #   cell (disabled input).
    # ---------------------------------------------------------------------

    _ELIGIBLE_BY_DONOR: Dict[str, set] = {}
    for _d in B1_DONORS:
        _ELIGIBLE_BY_DONOR[_d] = {_r for _r in B2_RECEIVERS if is_transition_eligible(_d, _r)}
    ...
    if "receiver_split_pivot" not in st.session_state:
        ...
        for _d in B1_DONORS:
            ...
            if n_elig > 0:
                # Start with equal shares; the last eligible receiver becomes remainder.
    ...
    for _d in B1_DONORS:
        ...
        remainder_r = (B2_RECEIVERS[-1] if (B2_RECEIVERS and B2_RECEIVERS[-1] in elig_set) else (elig[-1] if len(elig) > 0 else None))
        ...
        # Track running sum across eligible receivers except remainder
        running = 0.0
...
effs = [round(float(partner_share) / 100.0, 6)]
...
def _attach_transition_based_assets(sim: pd.DataFrame, roe: pd.DataFrame) -> pd.DataFrame:
    ...
    For each step:
      ...
      3) Annual -> total:
           Assets_offloaded_step_Tot equals Assets_offloaded_step_Yr in steady-state (no horizon)
    ...
    piv["Assets_Offloaded_ROE_Transition_EUR_bn_Tot"] = piv["Assets_Offloaded_ROE_Transition_EUR_bn_Yr"]
    piv["Assets_Offloaded_CET1_Transition_EUR_bn_Tot"] = piv["Assets_Offloaded_CET1_Transition_EUR_bn_Yr"]
    piv["Assets_Offloaded_Transition_EUR_bn_Tot"] = piv["Assets_Offloaded_Transition_EUR_bn_Yr"]
...
with left_col:
    ...
    with top_left:
        ...
        def _compute_max_reg_divergence_map_endstate():
            ...
            for _, b in banks_sel.iterrows():
                ...
                partner_share_dec = float(partner_share) / 100.0
    ...
    with top_right:
        ...
        def _plot_roe():
            ...
            for bank in BANK_ORDER:
                ...
                pot_vals = [max(max_ann - float(v), 0.0) for v in cur_vals] if np.isfinite(max_ann) else [0.0 for _ in cur_vals]
        ...
        st.plotly_chart(_plot_roe(), use_container_width=True)
    ...
    with bottom_left:
        ...
        fig1_yr = _build_offload_fig(
            ...
            title="Required offload (transition-based assets): CET1 + ROE split (annual / steady-state)",
        )
